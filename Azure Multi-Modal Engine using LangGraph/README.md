...
![DVC](https://img.shields.io/badge/DVC-Pipeline-945DD5?style=for-the-badge&logo=dvc&logoColor=white)
![License](https://img.shields.io/badge/License-MIT-green?style=for-the-badge)

**An end-to-end production-grade MLOps pipeline for automated kidney tumor classification from CT scan images, built with VGG16 transfer learning.**
...
This system classifies kidney CT scan images into two categories — **Tumor** and **Normal** — using a fine-tuned **VGG16** convolutional neural network. The project is architected as a production-grade MLOps pipeline: every training run is version-controlled, every experiment is tracked, and every deployment is fully automated.

### Key Capabilities

| Feature | Description |
|---|---|
| 📥 Data Ingestion | Automated dataset download from Google Drive via `gdown` |
| 🧬 Transfer Learning | Frozen VGG16 backbone with a custom classification head |
| 📊 Experiment Tracking | MLflow experiments logged to DagsHub |
| 🗄️ Metadata Persistence | Experiment metadata stored to MongoDB Atlas |
| 🔁 Checkpointing | Model checkpointing and TensorBoard callback support |
| 🌐 REST API | Flask endpoint for real-time inference |
| 🚀 Cloud Deployment | Containerised deployment on AWS EC2 via GitHub Actions + Amazon ECR |
...
A human-readable snapshot of every run — including scores, hyperparameters, artifact paths, and timestamp — is written to `metadata.txt` for safe Git storage, avoiding large binary rendering issues on GitHub.
...
- **Grad-CAM Visualisation** — Overlay heatmaps on CT scans to highlight regions driving the prediction
...
- **Mobile API** — Lightweight TFLite model export for on-device mobile deployment
...
*Building end-to-end machine learning systems with a focus on MLOps, computer vision, and cloud-native deployment.*

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Prithu)
